...
function DashboardPage() {
  ...
  return (
    <main className="pt-[64px] pl-[260px] h-screen overflow-y-auto relative">
      <div className="p-container_padding space-y-stack_lg pb-24">
        ...
        <div className="flex items-end justify-between">
          ...
          <div className="flex items-center gap-2 bg-secondary-container/30 px-4 py-2 rounded-lg border border-secondary-container">
            <span
              className={`w-2.5 h-2.5 rounded-full ${stats.isCajaOpen ? "bg-secondary animate-pulse" : "bg-error"}`}
            ></span>
            <span
              className={`font-label-md text-label-md font-bold ${stats.isCajaOpen ? "text-secondary" : "text-error"}`}
            >
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-gutter">
          ...
          <div className="bg-primary border border-primary p-stack_lg rounded-xl flex flex-col justify-center relative overflow-hidden">
            <div className="relative z-10 text-white">
              ...
              <Link
                to="/paquetes"
                className="font-caption text-caption mt-2 text-primary-fixed underline cursor-pointer hover:text-white transition-colors block"
              >
                  ...
                  {movements.length === 0 ? (
                    ...
                    movements.map((mov: any) => (
                      <tr key={mov.id} className="hover:bg-[#F4F6F8] transition-colors">
                        ...
                        <td
                          className="px-6 py-4 font-body-md text-body-md truncate max-w-[200px]"
                          title={mov.concepto}
                        >
                          {mov.concepto}
                        </td>
                        <td className="px-6 py-4">
                          <span
                            className={`px-2 py-1 text-[11px] font-bold rounded uppercase ${getCategoryColor(mov.categoria)}`}
                          >
                            {mov.categoria || mov.tipo}
                          </span>
                        </td>
                        <td
                          className={`px-6 py-4 font-body-md text-body-md font-bold ${mov.tipo === "Egreso" ? "text-error" : "text-secondary"}`}
                        >
                          {mov.tipo === "Egreso" ? "-" : "+"}
                        ...
                        <td className="px-6 py-4">
                          <div className="flex items-center gap-2">
                            <span
                              className={`w-2 h-2 rounded-full ${mov.estado === "Completado" ? "bg-secondary" : "bg-tertiary-fixed"}`}
                            ></span>
          ...
          <div className="space-y-gutter">
            ...
            <div className="bg-surface-container-lowest border border-outline-variant rounded-xl overflow-hidden shadow-sm">
              ...
              <div className="p-4 space-y-4">
                  ...
                  debts.map((deuda: any) => {
                    ...
                    return (
                      <div key={deuda.id} className="flex justify-between items-center group">
                        <div>
                          <p className="font-body-md text-body-md font-medium truncate max-w-[150px]">
                            {deuda.paciente?.nombre || "Desconocido"}
                          </p>
                ...
                <Link
                  to="/paquetes"
                  className="w-full block text-center py-2 mt-2 bg-surface-container-high text-on-surface-variant font-label-md text-label-md rounded-lg hover:bg-outline-variant transition-all"
                >
            ...
            <div className="bg-surface-container-lowest border border-outline-variant rounded-xl overflow-hidden shadow-sm">
              ...
              <div className="p-4 space-y-4">
                      ...
                      <div key={comision.id} className="flex items-center gap-3">
                        ...
                        <div className="flex-1 min-w-0">
                          <div className="flex justify-between">
                            ...
                            <p className="font-label-md text-label-md font-bold text-on-surface ml-2">
                ...
                <Link
                  to="/comisiones"
                  className="w-full block text-center py-2 mt-2 border border-primary text-primary font-label-md text-label-md rounded-lg hover:bg-primary/5 transition-all"
                >
                  Generar Pagos
                </Link>
              </div>
      ...
      <Link
        to="/movimientos"
        className="fixed bottom-8 right-8 w-14 h-14 bg-[#004286] text-white rounded-xl shadow-lg hover:shadow-xl hover:-translate-y-1 transition-all duration-300 flex items-center justify-center active:scale-95 z-50"
      >
